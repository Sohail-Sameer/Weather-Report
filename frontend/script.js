...
function renderMapScreen() {
  ...
  const pinCentralTemp = document.getElementById("pin-central-temp");
  const pinNorthTemp = document.getElementById("pin-north-temp");
  const pinEastTemp = document.getElementById("pin-east-temp");
  const pinCentralName = document.getElementById("pin-central-name");

  if (pinCentralTemp) pinCentralTemp.textContent = `${temp}°`;
  if (pinNorthTemp) pinNorthTemp.textContent = `${temp - 2}°`;
  if (pinEastTemp) pinEastTemp.textContent = `${temp + 2}°`;
  if (pinCentralName) pinCentralName.textContent = state.currentLocation.name;

  if (els.sensorMetricTemp) els.sensorMetricTemp.textContent = `${temp}°C`;
  if (els.sensorMetricWind) els.sensorMetricWind.textContent = `${Math.round(current.wind_speed_10m ?? 14)} km/h`;
  if (els.sensorMetricPrecip) els.sensorMetricPrecip.textContent = `${(current.precipitation ?? 0).toFixed(1)} mm`;
  if (els.sensorMetricAqi) els.sensorMetricAqi.textContent = state.airQuality?.us_aqi || 32;
...
// Camera photo analysis trigger
els.cameraUploadInput?.addEventListener("change", (e) => {
  const file = e.target.files?.[0];
  if (!file) return;
  appendUserChatBubble(`[Photo Attached: ${file.name}] Can you analyze the cloud formation in this sky?`);
  submitAssistantQuery(`I took a photo of the sky. Looking at the cloud formation and current weather in ${state.currentLocation.name}, what type of precipitation or front should I anticipate?`);
});
...
document.querySelectorAll(".map-layer-pill").forEach((pill) => {
  pill.addEventListener("click", () => {
    ...
    // Shift plume or simulation appearance
    if (els.mapRadarPlume) {
      if (state.activeMapLayer === "precip") {
        els.mapRadarPlume.setAttribute("opacity", "1");
      } else if (state.activeMapLayer === "temp") {
        els.mapRadarPlume.setAttribute("opacity", "0.4");
      } else {
        els.mapRadarPlume.setAttribute("opacity", "0.7");
      }
    }
  });
});

// Interactive Pinpoint Clicks
document.querySelectorAll(".map-pin").forEach((pin) => {
  pin.addEventListener("click", () => {
    const pinType = pin.dataset.pin;
    const temp = Math.round(state.weather?.current?.temperature_2m ?? 24);

    if (els.sensorCardTitle) {
      els.sensorCardTitle.textContent =
        pinType === "central"
          ? `${state.currentLocation.name} Core Cell`
          : pinType === "north"
          ? "Breeze Highlands Node"
          : "Valley Basin Telemetry Cell";
    }

    if (els.sensorMetricTemp) {
      els.sensorMetricTemp.textContent =
        pinType === "central" ? `${temp}°C` : pinType === "north" ? `${temp - 2}°C` : `${temp + 2}°C`;
    }
...
els.radarPlayBtn?.addEventListener("click", () => {
  ...
  if (isRadarPlaying) {
    ...
    radarInterval = setInterval(() => {
      ...
      // Animate plume translation
      if (els.mapRadarPlume) {
        const shiftX = (val / 60) * 40;
        els.mapRadarPlume.setAttribute("transform", `translate(${shiftX}, 0)`);
      }
    }, 450);
  } else {
