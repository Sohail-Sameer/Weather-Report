...
| 7 | Stylized interactive "radar" map view driven by live current-weather values | Radar Map tab |
...
The frontend renders this structured JSON as rich chat cards. If the model's JSON response fails to parse, the backend automatically retries with a simpler plain-text prompt and wraps the result in a default card structure so the UI never breaks. A camera/photo-attach button in the chat composer lets the user attach a sky photo; this currently triggers a pre-written text query about cloud formations rather than performing true image analysis (no image data is sent to any API).

### 5.8 Radar Map View
The Radar Map tab shows a stylized, hand-illustrated SVG "map" (coastline, grid, glow plumes, wind streaks) with three location pins (Downtown Core, Breeze Highlands, Valley Basin). The center pin's temperature is the actual current temperature; the other two pins are offset by ±2 °C for visual variety rather than reflecting independently fetched data. The Layer toggle pills (Precipitation, Micro-Temp, Wind Vectors, AQI Plume, Satellite) only switch which pill is marked "active" and adjust the opacity of the single precipitation-plume graphic — they do not load or display distinct real data layers. The bottom playback slider animates that same plume graphic sliding back and forth on a timer when "play" is pressed; it does not play back real historical or forecast radar frames. This entire view is a decorative/illustrative visualization built from the same live current-weather payload, not a connection to a real radar tile or satellite imagery service.
...
`backend/requirements.txt` includes `gunicorn`, a production-grade WSGI server, indicating the project is intended to be deployable behind a process manager rather than only the Flask development server. However, the provided project files do **not** contain a Render-specific configuration file (no `render.yaml`, `Procfile`, or similar), so there is no confirmed, pre-existing Render deployment setup in this codebase. To deploy on a platform such as Render, the following configuration would be needed:

- **Build command:** `pip install -r backend/requirements.txt`
- **Start command:** `gunicorn app:app --chdir backend --bind 0.0.0.0:$PORT`
- Configure the environment variables from [Section 13](#13-environment-variables--configuration) in the platform's dashboard rather than committing a `.env` file.

Since Flask serves the frontend as static files from the same app, a single web-service deployment is sufficient — no separate static-site hosting is required.
...
- Wire the "attach photo" feature to genuine image analysis (e.g., sending the photo to a vision-capable model) instead of triggering a fixed text prompt.
- Replace the illustrative SVG radar map with a real precipitation/radar tile layer for the selected location.
...
- Add a `render.yaml`/`Procfile` and CI pipeline for reproducible deployment.
